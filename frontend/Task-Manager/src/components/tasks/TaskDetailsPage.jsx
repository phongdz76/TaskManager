...
export default function TaskDetailsPage({
	...
	const handleUpdateStatus = async (newStatus) => {
		...
		try {
			const response = await axiosInstance.put(API_PATHS.TASKS.UPDATE_STATUS(task._id), {
				status: newStatus,
			});
			...
			<div className="max-w-7xl mx-auto pt-4 pb-10 animate-fade-in">
				<div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
					<div className="flex items-center gap-4">
						...
						<div>
							...
							<p className="mt-2 text-gray-500">View full task info and manage checklist progress.</p>
						</div>
					</div>
				...
				{loading ? (
					<div className="min-h-[40vh] flex flex-col items-center justify-center">
						...
						<p className="mt-3 text-gray-500 font-medium">Loading task details...</p>
					</div>
				) : !task ? (
					<div className="bg-white border border-gray-100 rounded-2xl p-8 text-center shadow-[0_2px_10px_rgba(0,0,0,0.04)]">
						<p className="text-gray-600 font-medium">Task not found or you do not have access.</p>
						<button
							onClick={() => navigate(backPath)}
					...
					<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
							...
							<div className="bg-white p-6 rounded-2xl shadow-[0_2px_10px_rgba(0,0,0,0.04)] border border-gray-100">
								...
								<div className="space-y-4">
									<div>
										...
										<p className="text-lg font-semibold text-gray-800 mt-1">{task.title || "Untitled task"}</p>
									</div>

									<div>
										<p className="text-xs uppercase tracking-wider text-gray-400 font-semibold">Description</p>
						...
						<div className="space-y-6">
							<div className="bg-white p-6 rounded-2xl shadow-[0_2px_10px_rgba(0,0,0,0.04)] border border-gray-100">
								...
								<div className="space-y-4">
									<div>
										<p className="text-xs uppercase tracking-wider text-gray-400 font-semibold mb-2">Current Status</p>
										<select
									...
									<div>
										<p className="text-xs uppercase tracking-wider text-gray-400 font-semibold mb-2">Priority</p>
										<span
									...
									<div>
										<p className="text-xs uppercase tracking-wider text-gray-400 font-semibold mb-2">Progress</p>
							...
							<div className="bg-white p-6 rounded-2xl shadow-[0_2px_10px_rgba(0,0,0,0.04)] border border-gray-100">
								<h3 className="text-lg font-bold text-gray-800 mb-4">Timeline & People</h3>
								...
								<div className="space-y-4 text-sm text-gray-700">
									...
									<div className="flex items-center gap-3">
										...
										<div>
											...
											<p className="font-medium">{formatTaskDate(task.dueDate)}</p>
										</div>
									</div>
									...
									<div className="flex items-start gap-3">
										...
										<div className="w-full">
											<p className="text-xs text-gray-400 uppercase tracking-wider">Assignees</p>
											{Array.isArray(task.assignedTo) && task.assignedTo.length > 0 ? (
												<div className="mt-2 space-y-2">
													{task.assignedTo.map((assignee) => {
														const name = assignee?.username || assignee?.name || assignee?.email || "Unknown";
														return (
															<div key={assignee?._id || name} className="flex items-center gap-2">
									...
									<div className="flex items-center gap-3">
										...
										<div>
											...
											<p className="font-medium">
												{task?.createdBy?.username || task?.createdBy?.name || task?.createdBy?.email || "Unknown"}
											</p>
										</div>
									</div>
								</div>
